...
delta.hedge <- function(M,N,S0,K,r,sigma,t,mu,alpha=1,call=1){
  print(N)
  ...
  delta <- function(M,n,t,r,S0,K,sigma,ss=1,call=1){
    set.seed(ss)
  ...
  IN <- which(X[,N+1] > K)
  CF[IN,N+1] <- K - ((1-deltas[IN,N])*X[IN,N+1]*(1+0.01))
  CF[-IN,N+1] <- deltas[-IN,N]*X[-IN,N+1]*(1-0.01)
  ...
  return(outlist)
    ...
    IN <- which(X[,N+1] < K)
    CF[IN,N+1] <- (-(((-1)-deltas[IN,N])*X[IN,N+1]))*(1-0.01)-K
    CF[-IN,N+1] <-(-(deltas[-IN,N]*X[-IN,N+1]))*(1+0.01)
    ...
    # compute performace
    H.perf <- sqrt(var(PV))/BLS
    outlist <- list("H.perf"=H.perf,"PV"=PV,"BLS"=BLS,"CF"=CF)
    return(outlist)
  }
}
...
for (j in 1:length(alpha)){
  sigma <- vol*S0^(1-alpha[j])
  tmp <- delta.hedge(M,N,S0,K,r,sigma,t,mu,alpha[j],call=0)
  H[j] <- tmp$H.perf
  PV[j] <- mean(tmp$PV)
}
